 ...
 */
app.controller('schoolManageCtrl',['$scope','$http','$filter','$state','$stateParams','$uibModal','httpService','SweetAlert','LoadingService','localStorageService',
    function($scope, $http,$filter,$state,$stateParams,$uibModal,httpService,SweetAlert,LoadingService,localStorageService) {
        ...
        $scope.queryList=function () {
            ...
            httpService.getAll(params,'school/showSchool').then(function (result) {
        ...
        $scope.delRow=function (id) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    httpService.delRow(params, 'school/delSchool').then(
                        function (result) {
                            if (result==1) {
                                $scope.reset();
        ...
        $scope.delRows=function () {
            if($scope.checked.length<0||$scope.checked.length===0){
                ...
                SweetAlert.swal({
                    ...
                    if (isConfirm) {
                        httpService.delRows(params, 'school/delSchools').then(
                            function (result) {
                                if (result) {
                                    $scope.reset();
                                    $scope.select_all = false;
