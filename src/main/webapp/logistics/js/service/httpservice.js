...
service.service('httpService', ['$rootScope','$q','$http', 'Upload','LoadingService','$timeout','SweetAlert',function($rootScope,$q,$http,Upload,LoadingService,$timeout,SweetAlert) {
    ...
    //创建post JSON请求
    this.c_post_json = function(url,params) {
        var timer = $timeout(function () {
            LoadingService.hide();
            SweetAlert.swal("网络异常,请稍后再试...", "", "warning");
        },5000);
    ...
    this.c_post = function(url,params) {
        ...
        var url = this.webroot+url;
        var params = this.handleParams(params);
        var promise = $http.post(url, params,timer);
        promise.then(
            // 通讯成功的处理
            function (answer) {
                //在这里可以对返回的数据集做一定的处理,再交由controller进行处理
    ...
    this.c_get = function(url,params){
        ...
        var url = this.webroot+url;
        var params = this.handleParams(params);
        var promise = $http.get(url, {params: params},{timeout: timer});
        promise.then(
            // 通讯成功的处理
            function (answer) {
                //在这里可以对返回的数据集做一定的处理,再交由controller进行处理
    ...
    /*controller的接口*/
    /*json 数据传输*/
    this.postJson = function(params,url){
        return this.c_post_json(url,params);
    };
