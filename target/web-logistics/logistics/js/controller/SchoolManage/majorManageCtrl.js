...
app.controller('majorManageCtrl',['$scope','$http','$filter','$state','$stateParams','$uibModal','httpService','SweetAlert','LoadingService','localStorageService',
    function($scope, $http,$filter,$state,$stateParams,$uibModal,httpService,SweetAlert,LoadingService,localStorageService) {
        ...
        $scope.queryList=function () {
            ...
            httpService.getAll(params,'major/showMajor').then(function (result) {
        ...
        $scope.delRow=function (id) {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    httpService.delRow(params, 'major/delMajor').then(
                        function (result) {
                            if (result==1) {
                                $scope.reset();
        ...
        $scope.delRows=function () {
            if($scope.checked.length<0||$scope.checked.length===0){
                ...
                SweetAlert.swal({
                    ...
                    if (isConfirm) {
                        httpService.delRows(params, 'major/delMajors').then(
                            function (result) {
                                if (result) {
                                    $scope.reset();
                                    $scope.select_all = false;
      ...
      //新增
        $scope.addMajor=function () {
            var modalInstance=$uibModal.open({
                ...
                keyboard:false,
                animation: true,
                templateUrl:'majorForm.html',
                controller:'addmajorModalCtrl'
